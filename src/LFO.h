...
public:
    ...
    void setParameters(float amplitude_, float frequency_, float phase_, Shape shape_);
...
inline LFO::LFO(const float amplitude_, const float frequency_, const float phase_, const Shape shape_)
{
...
inline void LFO::setPhase(const float phase_)
{
    phase = phase_;
}

inline void LFO::setShape(const Shape shape_)
{
    shape = shape_;
}

inline void LFO::setParameters(const float amplitude_, const float frequency_, const float phase_, const Shape shape_)
{
    amplitude = amplitude_;
    frequency = frequency_;
    phase = phase_;
...
inline float LFO::getAmplitude() const
{
   return amplitude;
}

inline float LFO::getFrequency() const
{
    return frequency;
}

inline float LFO::getPhase() const
{
    return phase;
}

inline LFO::Shape LFO::getShape() const
{
    return shape;
}
